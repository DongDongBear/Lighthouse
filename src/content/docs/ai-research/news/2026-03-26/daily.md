---
title: "2026-03-26 AI 日报：Anthropic 起诉五角大楼推出 Mac 控制，OpenAI 关停 Sora，Arm 发首颗 AI CPU"
description: "三大厂集体发力：Anthropic 起诉五角大楼 + Mac Computer Use + 科学博客；OpenAI 突然关停 Sora；Google 发 Gemini 3.1 Flash-Lite；Arm × Meta 发布 AGI CPU；Jensen Huang 宣称 AGI 已实现；LiteLLM 供应链攻击；Signal 创始人加密 Meta AI；AI 音乐欺诈认罪"
---
...
**概述：** Google 于 3 月 3 日发布 Gemini 3.1 Flash-Lite，定位为 Gemini 3 系列中最快、最具性价比的模型。价格仅 $0.25/百万输入 token、$1.50/百万输出 token，在 Arena.ai Leaderboard 上获得 Elo 1432 评分，在 GPQA Diamond（86.9%）和 MMMU Pro（76.8%）上超越了前代 Gemini 2.5 Flash，甚至超过部分更大的模型。
...
**信源：** https://www.apple.com/newsroom/2026/03/introducing-apple-business-a-new-all-in-one-platform-for-businesses-of-all-sizes/
...
- 两个项目都标记了 `/claude` 作为 co-builder，印证了 Claude Code 在开源开发中的主导地位
...
1. **Anthropic vs 五角大楼法律案裁决？** Rita Lin 法官即将做出初步禁令决定——结果将对 AI 行业与政府关系产生深远影响。Claude Computer Use 的实际可靠性数据如何？
2. **OpenAI Sora 关停的后续影响？** Disney 项目如何处理？现有 Sora API 用户的迁移方案？已发布的 Sora 2 API 功能是否继续维护？OpenAI 视频能力是否整合进 GPT 系列？
3. **Google I/O 2026（5 月 19-20 日）的 Agent 产品发布？** Project Mariner 团队重组后会推出什么产品？Gemini 3.1 Flash-Lite 正式版何时 GA？Walmart × OpenAI 协议最终走向如何？
